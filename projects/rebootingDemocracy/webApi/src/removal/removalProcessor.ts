...
export class RebootingDemocracyDeletionProcessor {
  ...
  private documentStore: PsRagDocumentVectorStore;
  private chunkStore: PsRagChunkVectorStore;
  ...
  async deleteDocumentAndChunks(
    ...
    if (!chunks || chunks.length === 0) {
      ...
      const chunkIds = chunks.map((chunk: any) => chunk._additional.id);
